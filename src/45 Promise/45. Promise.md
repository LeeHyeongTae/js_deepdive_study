...
promiseGet(wrongUrl).then(
  res => console.log(res),
  err => console.error(err)
); // 두번째 콜백 함수는 첫 번째 콜백 함수에서 발생한 에러를 캐치하지 못한다.
```
...
- 45-1-1. '콜백헬'에서와 같이 비동기 처리를 위한 콜백 패턴은 콜백 헬이 발생하는 문제가 있다.
- 프로미스는 then, catch, finally 후속 처리 메서드를 통해 콜백 헬을 해결한다.

```javascript
const url = 'https://jsonplaceholder.typiconde.com';
...
- then -> then -> chatch 순서로 후속 처리 메서드를 호출.
...
- 프로미스 후속 처리 메서드 없이 마치 동기 처리처럼 프로미스가 처리 결과를 반환하도록 구현할 수 있다.
...
- 프로미스는 주로 생성자 함수로 사용되지만 함수도 객체이므로 메서드를 가질 수 있다. 5개의 정적 메서드를 제공한다.
...
- 앞 선 비동기처리가 완료하면 다음 비동기 처리를 수행해서, 소요시간이 6초 이상이 된다.
...
- Promise.all 메서드는 프로미스를 요소 갖는 배열 등의 이터러블을 인수로 전달받는다.
- 전달받은 모든 프로미스가 모두 fulfilled상태가 되면 모든 처리 결과를 배열에 저장해 새로운 프로미스를 반환한다.
- 이때 첫 번째 프로미스가 resolve한 처리 결과부터 차례대로 배열에 저장해 그 배열을 resolve하는 새로운 프로미스를 반환한다. **(처리 순서 보장)**
...
- 인수로 전달받은 이터러블의 요소가 프로미가 아닌 경우 resolve 메서드를 통해 프로미스로 래핑한다.
...
Promise.all(githubIds.map(id => promiseGet(`https://api.github.com/users/${id}`)))
  // ['jeresig', 'ahejlsberg', 'ungmo2'] => Promise [userInfo, userInfo, userIfo]
...
- promiseGet 함수가 반환한 3개의 프로미스로 이루어진 배열을 인수로 전달 받고 Promise.all 메서드가 반환한 프로미스는 세개의 사용자 객체로 이루어진 배열을 담고 있다.
...
- 프로미스가 rejected 상태가 되면 Promise.all 메서드와 동일하기 처리된다.
...
- ES11(2020)에 도입된 메서드로, IE를 제외한 대부분의 모던 브라우저에서 지원한다.
...
- allSettled 메서드가 반환한 배열에는 fulfilled, rejected 상태와는 상관없이 인수로 전달받은 모든 프로미스들의 처리 결과가 담겨있다.
- fulfilled 상태인 경우, { status: string, value: any }를 갖는다.
- rejected 상태인 경우, { status: string, reason: string }를 갖는다.
...
- 마이크로태스크 큐는 별도의 큐로 프로미스의 후속 처리 메서드의 콜백 함수가 일시 저장된다.
...
- XMLHttpRequest 객체보다 사용방법이 간단하고 프로미스를 지원하기 때문에 비동기 처리를 위한 콜백 패턴의 단점에서 자유롭다.
...
- 후속 처리 메서드를 통해 프로미스가 resolve한 Response 객체를 전달받을 수 있다.
- Response.prototype에는 Response객체에 포함되어 있는 다양한 메서드를 제공한다.
...
- catch 후속 처리 메서드에 의해 'error'가 출력될 것 처럼 보이지만 'ok'가 출력된다.
- **fetch 함수가 반환하는 프로미스는 기본적으로 404 Not Found나 500 Internal Server Error와 같은 HTTP에러가 발생해도 에러를 reject하지 않고 불리언 타입의 ok 상태를 false로 설정한 Response 객체를 resolve한다. 오프라인 등의 네트워크 장애나 CORS 에러에 의해 요청이 완료되지 못한 경우에만 프로미스를 reject한다.**
...
- fetch 함수에 대한 더 자세한 내용은 [Fetch 사용하기](https://developer.mozilla.org/ko/docs/Web/API/Fetch_API/Using_Fetch)를 참고.
